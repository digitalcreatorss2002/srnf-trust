...
const programsData = [
  {
    ...
    isMiddle: false, // Wireframe position handler
  },
  {
    id: 2,
    ...
    isMiddle: true, // Beech wala card niche rahega wireframe ki tarah
  },
  {
    id: 3,
...
const OurPrograms = () => {
  ...
  // Auto slide loop for slider functionality (Changes every 5 seconds)
  useEffect(() => {
    if (isPaused) return;
  ...
  // Get active 3 cards subset to maintain infinite circular feeling
  ...
  return (
    <section className="w-full bg-[#f4f6f0] py-10 px-6 sm:px-10 lg:px-16 overflow-hidden">
      <div className="max-w-7xl mx-auto relative">
        ...
        <div
          ...
          onMouseEnter={() => setIsPaused(true)} // Hover par slider cycle stop ho jayegi
          onMouseLeave={() => setIsPaused(false)} // Mouse hatne par slide fir shuru ho jayegi
        >
          {visibleCards.map((program, index) => {
            // Index 1 hamesha beech wala card hoga grid row ka
            const isCenterCard = index === 1;

            return (
              <div
                key={`${program.id}-${index}`}
                className={`w-full bg-white rounded-3xl overflow-hidden shadow-md hover:shadow-2xl transition-all duration-500 transform group flex flex-col justify-between min-h-[400px] border border-gray-100 ${
                  /* 🌟 Wireframe Alignment Logic: Index 1 (Middle Card) ko niche shift kiya 'translate-y-8' se */
                  isCenterCard ? "md:translate-y-10" : "md:-translate-y-2"
                }`}
              >
                ...
                <div className="relative h-56 w-full overflow-hidden bg-gray-100">
                  ...
                  />
                  {/* Hover dark subtle fade coating */}
                ...
                <div className="p-6 flex-grow flex flex-col justify-between text-left">
                  <div className="space-y-3">
                    <h3 className="text-xl font-bold text-gray-800 group-hover:text-[#75843a] transition-colors duration-300">
                      {program.title}
                    </h3>
                  ...
                  <div className="pt-4">
                    <Link
                      to={program.link}
                      className="inline-flex items-center text-sm font-bold text-[#75843a] hover:text-[#5e6b2e] transition-colors group/btn"
                    >
                      Read More
        ...
        {/* 🧭 NAVIGATION CONTROLS (Left & Right Arrows with wider side gap) */}
        <div className="absolute top-[50%] -left-4 -right-4 md:-left-10 md:-right-10 lg:-left-16 lg:-right-16 transform -translate-y-1/2 flex justify-between pointer-events-none z-20">
          <button
            onClick={handlePrev}
            className="w-12 h-12 rounded-full bg-white hover:bg-[#75843a] text-gray-800 hover:text-white flex items-center justify-center shadow-lg transition-all transform hover:scale-110 active:scale-95 pointer-events-auto focus:outline-none"
          >
