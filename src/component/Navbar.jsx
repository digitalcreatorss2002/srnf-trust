...
const Navbar = () => {
  ...
  // Helper check function: checks if a specific link path matches current active window state
  const isLinkActive = (itemPath, hasDropdown = false, dropdownItems = []) => {
    ...
    // Exact matching logic for direct path strings
    if (currentFullRoot === itemPath) return true;
    
    // Base parent section matching rules
    if (location.pathname === itemPath) return true;
    
    // Dropdown match check: agar child items active hain, toh parent title auto highlighted rahega
